import React, { useState } from "react"
import { Modal, ModalBody, ModalHeader } from "reactstrap";
import logo from './cover.png'
export default function Footer(){
var p=new Date();
let year=p.getFullYear();
const [isModalOpen, toggleModal] = useState(false);
  return(
      <footer class="site-footer">
      <div class="container">
        <div class="row">
          <div class="col-xs-6 col-md-3">
            <h6>Contact Us</h6>
            <ul class="footer-links">
              <li><a class="facebook" href="#"><i class="fa fa-envelope"></i> karthikreddyjannupalli@gmail.com</a></li>
              <li><a class="twitter" href="#"><i class="fa fa-envelope"></i> lrahul28@gmail.com</a></li>
            </ul>
          </div>
          <div class="col-xs-6 col-md-3">
            <h6>Useful Links</h6>
            <ul class="footer-links">
              <li><a href="http://geeksforgeeks.org">Geeks For Geeks</a></li>
              <li><a href="http://w3schools.com/">W3 Schools</a></li>
              <li><a href="http://tutorialspoint.com">Tutorials Point</a></li>
              <li><a onClick={()=>{toggleModal(!isModalOpen); return false;}} href='#' >Get Started With CodeX</a></li>
            </ul>
          </div>
          <div class="col-xs-6 col-md-3">
<img src={logo} width={340} style={{marginLeft:130}} />
        </div>
        </div>
        <hr/>
      </div>
      <div class="container">
        <div class="row">
          <div class="col-md-8 col-sm-6 col-xs-12">
            <p class="copyright-text">Copyright &copy;{year} <a href="/home">CODEX</a>.
            </p>
          </div>

          <div class="col-md-4 col-sm-6 col-xs-12">
            <ul class="social-icons">
              <li><a class="facebook" href="#"><i class="fa fa-facebook"></i></a></li>
              <li><a class="twitter" href="#"><i class="fa fa-twitter"></i></a></li>
              <li><a class="linkedin" href="#"><i class="fa fa-linkedin"></i></a></li>   
            </ul>
          </div>
        </div>
      </div>
      <Modal
          className="modal-dialog-centered formContent"
          isOpen={isModalOpen}
          toggle={()=>toggleModal(!isModalOpen)}
        >
          <ModalHeader toggle={()=>toggleModal(!isModalOpen)} cssModule={{'modal-title': 'w-100 text-center'}}>
            <h5 className="text-center">Login/SignUp</h5>
          </ModalHeader>
          <ModalBody className="offset-2 container">
            <a href="http://localhost:5000/auth/google" class="btn button">
              <div>
                <span class="svgIcon t-popup-svg">
                  <svg
                    class="svgIcon-use"
                    width="25"
                    height="37"
                    viewBox="0 0 25 25"
                  >
                    <g fill="none" fill-rule="evenodd">
                      <path
                        d="M20.66 12.693c0-.603-.054-1.182-.155-1.738H12.5v3.287h4.575a3.91 3.91 0 0 1-1.697 2.566v2.133h2.747c1.608-1.48 2.535-3.65 2.535-6.24z"
                        fill="#4285F4"
                      />
                      <path
                        d="M12.5 21c2.295 0 4.22-.76 5.625-2.06l-2.747-2.132c-.76.51-1.734.81-2.878.81-2.214 0-4.088-1.494-4.756-3.503h-2.84v2.202A8.498 8.498 0 0 0 12.5 21z"
                        fill="#34A853"
                      />
                      <path
                        d="M7.744 14.115c-.17-.51-.267-1.055-.267-1.615s.097-1.105.267-1.615V8.683h-2.84A8.488 8.488 0 0 0 4 12.5c0 1.372.328 2.67.904 3.817l2.84-2.202z"
                        fill="#FBBC05"
                      />
                      <path
                        d="M12.5 7.38c1.248 0 2.368.43 3.25 1.272l2.437-2.438C16.715 4.842 14.79 4 12.5 4a8.497 8.497 0 0 0-7.596 4.683l2.84 2.202c.668-2.01 2.542-3.504 4.756-3.504z"
                        fill="#EA4335"
                      />
                    </g>
                  </svg>
                </span>
                <span class="button-label">Sign in with Google</span>
              </div>
            </a>
            <br/>
            <br/>
            <br />
          </ModalBody>
        </Modal>
</footer>
    );
  }