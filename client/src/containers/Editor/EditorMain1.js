...
import { Alert, Dialog, Grid, Snackbar } from "@material-ui/core";
    ...
    if (this.state.value.trim() === "")
      ...
      credentials: "include"})
        ...
        if (this.state.value.trim() === "")
          ...
          this.scrollToBottom();
            Axios.post("http://" + properties.getHostName + ":5000/api/editor/runtestcases",JSON.stringify({
              source: this.state.value,
              lang: this.state.languageValue,
            ...
            .then((response) => {
              if (response)
                this.setState(
                  {
                    stdOut: response.codingAssignmentStatus,
                    actual: response.actual,
                    errorMessage: response.errorMessage,
                    buttonDisabled: false,
                    open: response.codingAssignmentStatus==="ACCEPTED" ? true: false,
                    compileError: response.compile_status==="OK"?"":response.compile_status,
                    stdOut: response.codingAssignmentStatus,
                    runtime: response.compile_status==="OK"?response.run_status.time_used:0,
                    memory: response.compile_status==="OK"?response.run_status.memory_used:0,
            ...
            .catch((err) => {
            console.log("Please login before you compile your program:"+err, "error");
